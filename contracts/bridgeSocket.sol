...
contract BridgeSocket is Context, ReentrancyGuard, Ownable {
    ...
    function getAssetLimits(address assetAddress)
        public
        view
    ...
    function bridgeAsset(
        ...
        if (isNativeAsset(assetAddress)) {
            if (assetAddress == address(0)) {
                transactionID = bridge.send{value: gas + _amount}(
                    chainID,
                    assetAddress,
                    _amount,
                    reciever
                ...
                transactionID = bridge.send{value: gas}(
                    chainID,
                    assetAddress,
                    _amount,
                    reciever
            ...
            IERC20(assetAddress).safeApprove(address(bridge), _amount);
            transactionID = bridge.burn{value: gas}(
                chainID,
                assetAddress,
                _amount,
                reciever
    ...
    function processFees(uint256 amount, address assetAddress)
        private
        returns (uint256)
    {
    ...
    function updateFeeRemitance(address _feeRemitance)
        public
        onlyOwner
        notPaused
    {
