...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function _update(address from, address to, uint tokenId) internal{

    uint[] memory tokenList = tokenIds[from];
        uint index;

        if (tokenList.length == 1) {
            delete tokenIds[from];
        }
        else{
        ...
        tokenIds[from][index] = tokenIds[from][tokenList.length-1];
        tokenIds[from].pop();
        }
