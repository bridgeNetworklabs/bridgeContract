...
//    address public immutable migrator;
   ...
   uint256  constant public standardDecimals = 18;
   ...
   event AssetUpdated(address indexed assetAddress ,address indexed  manager ,address indexed  feeRemitance ,uint256 min ,uint256 max ,bool native );
...
//    event NativeAssetStatusChanged(address indexed assetAddress , bool status);
  ...
  function pauseBrigde() external {
   ...
   function getAssetDecimals(address assetAddress) internal view returns (uint256 decimals) {
    ...
    function standaredize(uint256 amount,uint256 decimals) internal pure returns (uint256) {
    
       return amount * (10**(standardDecimals - decimals));
    }
    
    function convertToAssetDecimals(uint256 amount  ,uint256 decimals ) internal pure returns (uint256) {
    ...
    //    require(, "C_E");
       ...
       if(isDirectSwap[assetAddress][chainTo]){
           if(assetAddress == address(0)){
               ...
               IERC20(assetAddress).safeApprove(address(bridgePool), recievedValue);
               bridgePool.topUp(assetAddress , recievedValue);
           }
       }
       nativeAssets[assetAddress].balance += recievedValue;

       recievedValue = standaredize(recievedValue, getAssetDecimals(assetAddress));
        uint256 nounce;
        (transactionID , nounce) = registry.registerTransaction(chainTo , assetAddress , recievedValue , receiver  , 0);
   ...
   function burn(address assetAddress , uint256 amount ,  address receiver) external payable  nonReentrant returns (bytes32 transactionID){
       ...
       noneZeroAddress(receiver);
        (bool success, uint256 recievedValue) =  processedPayment(assetAddress ,chainTo, amount);
       require(success && recievedValue  >= foriegnAssets[assetAddress].minAmount && recievedValue  <= foriegnAssets[assetAddress].maxAmount, "I_F");
       ...
       recievedValue = standaredize(recievedValue, getAssetDecimals(assetAddress));
       uint256 nounce;
        (transactionID , nounce)  = registry.registerTransaction( chainTo , _foriegnAsset ,recievedValue , receiver , 1);
   ...
   function mint(bytes32 mintID) public  nonReentrant { 
    ...
    //    require(, "MI_E");
       ...
       uint256 amount  = convertToAssetDecimals(transaction.amount, getAssetDecimals(transaction.assetAddress));
       if (isDirectSwap[transaction.assetAddress][transaction.chainId]) {
           
           bridgePool.sendOut(transaction.assetAddress, transaction.receiver , amount);
       } else {
           IwrappedToken(transaction.assetAddress).mint(transaction.receiver , amount);
       }
   ...
   function claim(bytes32 claimID) public  nonReentrant {
    ...
    //    require( , "CI_E");
       ...
       uint256 amount  = convertToAssetDecimals(transaction.amount , getAssetDecimals(transaction.assetAddress));
       require(registry.isClaimTransaction(claimID) && registry.assetChainBalance(transaction.assetAddress,transaction.chainId) >= amount  && !transaction.isCompleted && registry.transactionValidated(claimID) ,"AL_E");
    //    require(, "C");
    //    require(, "N_V");
       
       payoutUser(payable(transaction.receiver), transaction.assetAddress ,  amount);
    ...
    function migrateForiegn(uint256 limit , bool directSwap ) external { 
        ...
        if(directSwap  ) {
            ...
            for (uint256 i; i < migrationAmount ; i++) { 
                directForiegnAsset storage directSwapAsset = directForiegnAssets[start + i];
       ...
       external 
    {
        ...
        require(settings.isNetworkSupportedChain(chainID) && !hasWrappedForiegnPair[foriegnAddress][chainID] && _msgSender() == migrator &&  wrappedAddress != address(0) , "U_A");
              ...
              if(directSwap){
                    ...
                    directForiegnAssets[directForiegnCount] = directForiegnAsset( wrappedAddress ,foriegnAddress, chainID, true);
                    directForiegnCount++;
                }
                else{
   ...
   function getAssetCount() external view returns (uint256  , uint256  , uint256 ) {
       return (nativeAssetsList.length,foriegnAssetsList.length ,directForiegnCount);
   ...
   function bridgeData() external view returns (address , address , bool  , uint256 , uint256 ,uint256 , uint256 , uint256 ,address ,address , uint256 , uint256){
       return( migrator , newBridge , activeMigration , migrationInitiationTime , migrationDelay ,fMigrationAt , fDirectSwapMigrationAt , nMigrationAt ,deployer , feeController , totalFees , feeBalance);
   }
}
