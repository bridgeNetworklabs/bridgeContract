...
//     }
  function onlyValidator() internal view returns (bool){
   ...
   function addNativeAsset(address assetAddress , uint256 minAmount , uint256[] calldata supportedChains , bool transferFee) public  {
        onlyAdmin();
      ...
      function registerRail(address assetAddress , uint256 minAmount , uint256[] calldata supportedChains , address feeAccount) public  {
          if(settings.onlyOwnableRail()){
              require( _msgSender() == IERCOwnable(assetAddress).owner() , "n_o");
          }
        //   else{
        //       require(settings.approvedToAdd(assetAddress , msg.sender), "n_a");
        //   }
   ...
   address feeAddress) public {
      require(controller.isAdmin(_msgSender()) || _msgSender() == controller.owner() || controller.isRegistrar(_msgSender()) ,'U_A');
       ...
       require(!hasWrappedForiegnPair[foriegnAddress][chainID] , "already registered");
       WRAPPEDTOKEN wrappedAddress = new WRAPPEDTOKEN(string(abi.encodePacked( "br", _name)) , string(abi.encodePacked("br" , _symbol)));
       foriegnAssets[address(wrappedAddress)] = foriegnAsset(address(wrappedAddress), foriegnAddress , minAmount ,fee, 0 ,0,OwnedRail , manager,feeAddress , chainID , true);
      
       wrappedForiegnPair[foriegnAddress][chainID] =  address(wrappedAddress);
       ForiegnPair[address(wrappedAddress)] = foriegnAddress;
   ...
   function registerMintTransaction(bytes32 mintID , uint256 chainFrom , address assetAddress , uint256 amount,  address receiver , uint256 nounce) public {
       ...
       require(foriegnAssets[ wrappedForiegnPair[assetAddress][chainFrom]].isSet , "asset_err");
       require(foriegnAssets[wrappedForiegnPair[assetAddress][chainFrom]].chainID == chainFrom , "chain_err");
       bytes32 requiredmintID = keccak256(
                                        abi.encodePacked(chainId, assetAddress , amount, receiver, nounce)
                                    );
        require(mintID  == requiredmintID, "mint: error validation mint ID");
        mintTransactions[mintID]  = Transaction(chainFrom , wrappedForiegnPair[assetAddress][chainFrom], amount , receiver, nounce, false);
        isMintTransactions[mintID] = true;
        pendingMintTransactions.push(mintID);
       
   }
   function burn( address assetAddress , uint256 amount ,  address receiver) public payable returns(bytes32){
       require(foriegnAssets[assetAddress].isSet , "has no foriegnAsset pair ");
       require(amount  >= foriegnAssets[assetAddress].minAmount , "amount below minimum");
       require(receiver != address(0) , "xant send to Zero address");
       require(processedPayment(assetAddress ,foriegnAssets[assetAddress].chainID, amount ) , "i_amount");
       deductFees(assetAddress , 0 , false);
       _burnToken(assetAddress ,amount);
      uint256 chainTo =foriegnAssets[assetAddress].chainID;
   ...
   // internal fxn for deducting and remitting fees after a sale
    ...
    require(foriegnAssets[assetAddress].isSet  , "has no foriegnAsset pair ");
    foriegnAsset storage asset = foriegnAssets[assetAddress];
    if(asset.transferFee ){
